...
local parser = require("plugins.neotest-bun.parse-result")
...
Adapter = {
	...
	build_spec = function(args)
		...
		-- Use absolute path for XML output to avoid cwd issues
		local root_dir = lib.files.match_root_pattern("package.json")(position.path) or vim.fn.getcwd()
		local xml_path = root_dir .. "/neotest-bun.xml"
		local command = { "bun", "test", "--reporter=junit", "--reporter-outfile=" .. xml_path }
	...
	results = function(spec, _result, tree)
		-- Use the absolute path stored in context during build_spec
		local xml_path = spec.context.xml_path
		local xml_content = readXMLFile(xml_path)
		os.remove(xml_path)
